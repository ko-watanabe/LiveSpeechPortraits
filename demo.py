...
if __name__ == '__main__':
    ...
    parser.add_argument('--id', default='May',
                        help="person name, e.g. Obama1, Obama2, May, Nadella, McStay")
    parser.add_argument(
        '--driving_audio', default='./data/input/00083.wav', help="path to driving audio")
    parser.add_argument('--save_intermediates', default=0,
                        help="whether to save intermediate results")
    parser.add_argument('--device', type=str, default='cpu',
                        help='use cuda for GPU or use cpu for CPU')
    ...
    std_mean_pts3d = np.load(
        config['dataset_params']['pts3d_path']).mean(axis=0)
    # candidates images
    img_candidates = []
    for j in range(4):
        output = imread(join(data_root, 'candidates',
                        f'normalized_full_{j}.jpg'))
    ...
    APC_model.load_state_dict(torch.load(
        config['model_params']['APC']['ckp_path'], map_location=torch.device('cpu')), strict=False)
    if opt.device == 'cuda':
        APC_model.cuda()
    APC_model.eval()
    ...
    with torch.no_grad():
        ...
        mel80_torch = torch.from_numpy(
            mel80.astype(np.float32)).to(device).unsqueeze(0)
        hidden_reps = APC_model.forward(mel80_torch, length)[
            0]   # [mel_nframe, 512]
    ...
    if use_LLE:
        ...
        weights, feat_fuse = utils.compute_LLE_projection_all_frame(
            audio_feats, APC_feat_database, ind, audio_feats.shape[0])
        audio_feats = audio_feats * (1-LLE_percent) + feat_fuse * LLE_percent

    # 3. Audio2Mouth
    print('3. Audio2Mouth inference...')
    pred_Feat = Audio2Feature.generate_sequences(
        audio_feats, sr, FPS, fill_zero=True, opt=Featopt)
    ...
    pred_Head = Audio2Headpose.generate_sequences(
        audio_feats, pre_headpose, fill_zero=True, sigma_scale=0.3, opt=Headopt)
    ...
    pred_pts3d = utils.landmark_smooth_3d(
        pred_pts3d, Feat_smooth_sigma, area='only_mouth')
    ...
    for k in tqdm(range(nframe)):
        ...
        final_pts3d[k, eye_brow_indices] = candidate_eye_brow[ind] + \
            mean_pts3d[eye_brow_indices]
    ...
    for ind in tqdm(range(0, nframe), desc='Image2Image translation inference'):
        ...
        if save_feature_maps:
            visual_list += [('input',
                             np.uint8(current_pred_feature_map[0].cpu().numpy() * 255))]
